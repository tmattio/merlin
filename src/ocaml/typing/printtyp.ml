...
module Names : sig
  val names : (transient_expr * string) list ref
...
let tree_of_cltype_declaration id cl rs =
  ...
  let virt =
    ...
    List.exists
      (fun (lab, _, _) ->
         not (lab = dummy_method || Meths.mem lab sign.csig_meths))
      fields
